...
const PostForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    postImageURL: initialData?.postImageURL || null,
  });
  ...
  const handleFeaturedImageUpload = async (e) => {
    ...
    try {
      const { imageUrl } = await postService.uploadPostImage(
        file,
        formData.postImageURL,
      );
      setFormData((prev) => ({ ...prev, postImageURL: imageUrl }));
  ...
  const removeFeaturedImage = () => {
    setFormData((prev) => ({ ...prev, postImageURL: null }));
  ...
  useEffect(() => {
    if (initialData) {
      ...
      if (initialData.postImageURL) {
        setPostImagePreview(initialData.postImageURL);
      }
    }
  }, [initialData]);
